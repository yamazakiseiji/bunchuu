...
/**********************************************************************/
/* SPEC;                                                              */
/*   NAME = cos: double precision cosine function;                    */
/*   FUNC = Computes double precision cosine of its argument;         */
/*   CLAS = UNIT;                                                     */
/*   NOTE = SH C/C++ fast floating point library;                     */
/* END;                                                               */
/**********************************************************************/
#if defined _FLT || defined _FPS        /* double=float or fpu=single */
#define cosf cos                        /* rename cosf to cos         */
#include "cosf.c"                       /* include cosf               */
#else
...
extern double cos(double);              /* FFPLB-017 (A)              */

static double cos_special(double d0){   /* FFPLB-017 (A)              */
  ...
  return cos(d0);                       /* FFPLB-017 (A)              */
}                                       /* FFPLB-017 (A)              */

double cos(double d0)
{
  ...
  if ((*(unsigned long *)&d0&EXP_MASK)>=0x41d00000) /* FFPLB-017 (C)  */
    return cos_special(d0);             /* FFPLB-017 (C)              */
#else                                   /* Little endian              */
